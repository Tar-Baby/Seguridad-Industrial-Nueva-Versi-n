...
//
//Function to blend 3 textures using an alpha mask with vertex colors.
//
//By:Steve Coyle
//Created: 8/24/2017
//------------------------------------------------------------------------

Shader "AllAccessAssets/SH_HeightBasedVertexBlendx3" {
	Properties{
		...
		_Color3("Color 03", Color) = (1,1,1,1)
		_MainTex3("Albedo 02 (RGB) Smooth (A)", 2D) = "white" {}
		_BumpMap3("Normal Map 02",2D) = "bump" {}
		[Gamma] _Metallic3("Metallic 03", Range(0.0, 1.0)) = 0.0
		_Glossiness3("Smoothness 03", Range(0,1)) = 0.5

		[KeywordEnum(Red,Green,Blue,Alpha)] _Channel("Vertex Channel Blend 1", Float) = 0
		[KeywordEnum(Red,Green,Blue,Alpha)] _Channel2("Vertex Channel Blend 2", Float) = 0
...
#pragma multi_compile _CHANNEL2_RED _CHANNEL2_BLUE _CHANNEL2_GREEN CHANNEL2_ALPHA
	...
	void surf(Input IN, inout SurfaceOutputStandard o) {
		...
		float3 n3 = UnpackNormal(tex2D(_BumpMap3, IN.uv_BumpMap3));
		float metallic3 = _Metallic3;
		float smoothness3 = _Glossiness3;
...
#ifdef _CHANNEL2_RED
		alpha2 = BlendAlphaVertex(IN.vColor.r, hm.r, hm.g);
#endif
#ifdef _CHANNEL2_GREEN
		alpha2 = BlendAlphaVertex(IN.vColor.g, hm.r, hm.g);
#endif
#ifdef _CHANNEL2_BLUE
		alpha2 = BlendAlphaVertex(IN.vColor.b, hm.r, hm.g);
#endif
#ifdef _CHANNEL2_ALPHA
		alpha2 = BlendAlphaVertex(IN.vColor.a, hm.r, hm.g);
#endif

		float3 finalColor1 = lerp(c2.rgb, c1.rgb, alpha);
		float3 finalColor2 = lerp(c3.rgb, finalColor1, alpha2);
		float m1 = lerp(_Metallic2, _Metallic, alpha);
		float m2 = lerp(_Metallic3, m1, alpha2);
		float s1 = lerp(_Glossiness2 *c2.a, _Glossiness*c1.a, alpha);
		float s2 = lerp(_Glossiness3 *c3.a, s1, alpha2);
		float3 finalnm1 = lerp(n2, n1, alpha);
		float3 finalnm2 = lerp(n2, finalnm1, alpha2);
		o.Normal = finalnm2;
